...
-- 设置震动
local file_vibrate = device.writablePath .. ".vibrate"
function device.setVibrate(enable)
    if enable then
        cc.FileUtils:getInstance():writeStringToFile("" , file_vibrate)
    else
        cc.FileUtils:getInstance():removeFile(file_vibrate)
    end
end

-- 是否启用震动
function device.isVibrate()
    return cc.FileUtils:getInstance():isFileExist(file_vibrate)
end

-- 震动
function device.vibrate(time)
    if device.isVibrate() then
