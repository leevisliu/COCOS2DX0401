...
function LogonFrame:onConnectCompeleted()
	...
	elseif self._logonMode == 5 then
		self:sendBindingPhone()
	elseif self._logonMode == 6 then
		self:sendGetPhoneNumber()
	elseif self._logonMode == 7 then
		self:sendUnBindPhone()
	else
		self:onCloseSocket()
...
function LogonFrame:onGetCode(szMobile)
	--数据保存
	self._szMobilePhone = szMobile
...
function LogonFrame:onBindingPhone(szMobile,szSMSCode,szBindingAccount)
	--数据保存
	self._szMobilePhone = szMobile
    self._szSMSCode = szSMSCode
    self._szAccount = szBindingAccount
	--记录模式
	self._logonMode = 5
	if not self:onCreateSocket(yl.LOGONSERVER,yl.LOGONPORT) and nil ~= self._callBack then
		self._callBack(-1,"建立连接失败！")
...
function LogonFrame:onGetPhoneNumber(szBindingAccount)
	--数据保存
    self._szAccount = szBindingAccount
	--记录模式
	self._logonMode = 6
...
function LogonFrame:onUnBindPhone(szMobile,szSMSCode,szBindingAccount)
	--数据保存
	self._szMobilePhone = szMobile
    self._szSMSCode = szSMSCode
    self._szAccount = szBindingAccount
...
function LogonFrame:sendGetCode()
	local LogonData = CCmd_Data:create(235+14)
	LogonData:setcmdinfo(yl.MDM_MB_LOGON,yl.SUB_MB_REGISTER_GETCODE)
	LogonData:pushword(--[[GlobalUserItem.nCurGameKind]]yl.INVALID_WORD)
	LogonData:pushdword(self._plazaVersion)
	LogonData:pushbyte(self._deviceType)
	LogonData:pushstring(self._szMobilePhone,yl.LEN_MOBILE_PHONE)
	...
	--发送失败
	if not self:sendSocketData(LogonData) and nil ~= self._callBack then
		self._callBack(-1,"获取验证码失败！")
	end
end
--绑定手机
function LogonFrame:sendBindingPhone()
	local LogonData = CCmd_Data:create(235+14)
	LogonData:setcmdinfo(yl.MDM_MB_LOGON,yl.SUB_MB_BINDING_PHONE)
	LogonData:pushword(--[[GlobalUserItem.nCurGameKind]]yl.INVALID_WORD)
	LogonData:pushdword(self._plazaVersion)
	LogonData:pushbyte(self._deviceType)
	LogonData:pushstring(self._szMobilePhone,yl.LEN_MOBILE_PHONE)
...
--获取手机号码
function LogonFrame:sendGetPhoneNumber()
	local LogonData = CCmd_Data:create(235+14)
	LogonData:setcmdinfo(yl.MDM_MB_LOGON,yl.SUB_MB_GET_PHONE_NNUMBER)
	LogonData:pushword(--[[GlobalUserItem.nCurGameKind]]yl.INVALID_WORD)
	LogonData:pushdword(self._plazaVersion)
	LogonData:pushbyte(self._deviceType)
	LogonData:pushstring(self._szMobilePhone,yl.LEN_MOBILE_PHONE)
	...
	--发送失败
	if not self:sendSocketData(LogonData) and nil ~= self._callBack then
		self._callBack(-1,"获取手机号码失败！")
	end
end

--取消绑定手机
function LogonFrame:sendUnBindPhone()
	local LogonData = CCmd_Data:create(235+14)
	LogonData:setcmdinfo(yl.MDM_MB_LOGON,yl.SUB_MB_UNBINDING_PHONE)
	LogonData:pushword(--[[GlobalUserItem.nCurGameKind]]yl.INVALID_WORD)
	LogonData:pushdword(self._plazaVersion)
	LogonData:pushbyte(self._deviceType)
	LogonData:pushstring(self._szMobilePhone,yl.LEN_MOBILE_PHONE)
