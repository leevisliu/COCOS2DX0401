...
function ClientScene:onCreate()
    ...
    self._gameLists = {
        --{ 510, 36, 6, 49 },
        { 49, 26, 36, 102, 27, 25, 200, 6, 238, 601, 389 },
        { 510, 507, 511 },
    ...
    self._gameListLayer = GameListLayer:create(self)
                                                :setContentSize(1334 + (yl.APPSTORE_VERSION and self._areaCategory:getContentSize().width or 0), 582)
                                                :setPosition(0, 87)
    ...
    for i = 1, 1 do
        ...
        btnRankCategory:setVisible(false);
        self._rankCategoryBtns[i] = btnRankCategory
    end
    self.btn_rank = areaRank:getChildByName("btn_rank");
    self.btn_rank:addClickEventListener(function()
        local x = areaRank:getPositionX();
        local y = areaRank:getPositionY();
        if x == 0 then
            areaRank:runAction(cc.MoveTo:create(0.5, cc.p(-365, y)));
        else
            areaRank:runAction(cc.MoveTo:create(0.5, cc.p(0, y)));
        end
    end)
    ...
    --排行榜列表
    self._rankListLayer = RankingListLayer:create(cc.size(320, 480))
                                :setDelegate(self)
                                :setAnchorPoint(0, 0)
                                :setPosition(28, 12)
                                :addTo(areaRank)
    ...
    for k, v in ipairs(self._areaBottom:getChildren()) do 
        print(v:getName());
    end 
    ...
    --福利按钮
    local btnWelfare = self._areaBottom:getChildByName("btn_welfare")
    ...
    --头像按钮
    local btnAvatar = self._areaTop:getChildByName("btn_avatar")
    ...
    --复制ID
    local btnCopyID = self._areaTop:getChildByName("btn_copyid")
    btnCopyID:addClickEventListener(function()
        
        --播放音效
        ExternalFun.playClickEffect()

        MultiPlatform:getInstance():copyToClipboard("亲，我的久游棋牌昵称："..GlobalUserItem.szNickName.."，ID："..GlobalUserItem.dwGameID)
        showToast(nil, "您的游戏ID昵称，已复制到剪切板", 2)
    end)

    --推荐人
    local btnSpreader = self._areaTop:getChildByName("btn_spreader")
    ...
    --增加游戏币按钮
    local btnAddGold = self._areaTop:getChildByName("area_gold_info"):getChildByName("btn_add_gold")
    ...
    --增加游戏豆按钮
    local btnAddBean = self._areaTop:getChildByName("area_bean_info"):getChildByName("btn_add_bean")
...
function ClientScene:onUpdateUserInfo()
    ...
    --设置玩家头像
    local avatar = self._areaTop:getChildByName("sp_avatar")
    if nil ~= avatar then
        avatar:updateHead(GlobalUserItem)
    else
        local avatarFrame = self._areaTop:getChildByName("sp_avatar_frame")
        avatarFrame:setLocalZOrder(1);
        HeadSprite:createClipHead(GlobalUserItem, 64, "sp_avatar_mask_64.png")
              :setPosition(avatarFrame:getPosition())
              :setName("sp_avatar")
              :addTo(self._areaTop, avatarFrame:getLocalZOrder() - 1)
    end

    --玩家昵称
    local txtNickName = self._areaTop:getChildByName("txt_nickname")
    txtNickName:setString(string.EllipsisByConfig(GlobalUserItem.szNickName, 200, self._nickNameFontConfig))
    --游戏ID
    local txtGameID = self._areaTop:getChildByName("txt_gameid")
    txtGameID:setString("ID:" .. GlobalUserItem.dwGameID)

--    local vipIdx = GlobalUserItem.cbMemberOrder or 0;
--    local vipSp = self._areaTop:getChildByName("vip_sprite")
--	if nil == vipSp then
--		vipSp = cc.Sprite:create("Information/atlas_vipnumber.png")
--		if nil ~= vipSp then
--			vipSp:setName("vip_sprite")
--			vipSp:setAnchorPoint(cc.p(0, 0.5))
--			vipSp:setPositionX(txtGameID:getPositionX() + txtGameID:getContentSize().width + 20)
--            vipSp:setPositionY(txtGameID:getPositionY())
--			self._areaTop:addChild(vipSp)			
--		end
--	end
--	vipSp:setScale(0.9)
...
function ClientScene:onUpdateScoreInfo()
    ...
    --游戏币
    local txtGold = self._areaTop:getChildByName("area_gold_info"):getChildByName("txt_gold")
    txtGold:setString(ExternalFun.numberThousands(GlobalUserItem.lUserScore))

    --游戏豆
    local txtBean = self._areaTop:getChildByName("area_bean_info"):getChildByName("txt_bean")
...
function ClientScene:onUpdateOnlineCount()
    ...
    onlineCount = onlineCount + GlobalUserItem.OnlineBaseCount + math.random(2200, 2400)
    ...
    self._areaRank:getChildByName("area_online_count"):setVisible(false);
end
...
function ClientScene:onClickGame(wKindID)
    ...
    AnimationHelper.moveOutTo(self._gameListLayer, 0.4, cc.p(self._ptGameListLayer.x + 1400, self._ptGameListLayer.y))
