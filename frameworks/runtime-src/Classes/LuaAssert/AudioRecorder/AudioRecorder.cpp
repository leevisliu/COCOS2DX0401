...
static int toLua_AudioRecorder_createSendBuffer(lua_State *tolua_S)
{
    ...
    if (nullptr != ret)
    {
        ...
        if (FileUtils::getInstance()->isFileExist(fileFullPath))
        {
            ssize_t size = 0;
			Data d;
            FileUtils::getInstance()->getContents(fileFullPath, &d);
			unsigned char * pData = d.takeBuffer(&size);
            if (0 != size)
            {
...
static int toLua_AudioRecorder_setFinishCallBack(lua_State *tolua_S)
{
    ...
    if (nullptr != ret)
    {
        ...
        if (argc == 4)
        {
			std::string audiopath = (std::string)lua_tostring(tolua_S, 2);
            int audioId = (int)lua_tointeger(tolua_S, 3);
            auto nHandler = toluafix_ref_function(tolua_S, 4, 0);
			RDAudio::getInstance()->loadFileAsyn(audiopath.c_str(), audioId,[](int aid, ALuint bufferID)
			{
				lua_State* tolua_S = LuaEngine::getInstance()->getLuaStack()->getLuaState();
				lua_pushinteger(tolua_S, aid);
				lua_pushinteger(tolua_S, bufferID);
				LuaEngine::getInstance()->getLuaStack()->executeFunctionByHandler(aid, 3);
			});
            return 1;
        }
    }
